...
export const MarketTicker = ({ items }: { items: TickerItem[] }) => {
  ...
  return (
    <div className="w-full bg-zinc-950/90 dark:bg-black/95 backdrop-blur-xl border-b border-white/10 py-1.5 overflow-hidden whitespace-nowrap z-50 sticky top-0 shadow-lg">
      {/* Edge Fades */}
      ...
      <motion.div
        ...
        transition={{
          duration: items.length * 15, // Slower for premium feel
          ease: "linear",
          repeat: Infinity,
        }}
      ...
      >
        {loopedItems.map((item, idx) => (
          <div key={`${item.id}-${idx}`} className="inline-flex items-center gap-3">
            <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400/80">
              {item.label}
            </span>
            <span className="text-[13px] font-bold tabular-nums tracking-normal text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)]">
